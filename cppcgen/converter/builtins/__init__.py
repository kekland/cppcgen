...
def maybe_get_builtin_structure(type: cpp.Type) -> Optional[BuiltinStructure]:
  ...
  if type.namespaced_base_name == 'std::string': result = StdStringStructure()
  elif type.namespaced_base_name == 'std::vector': result = StdVectorStructure(arg=type.template_args[0])
  elif type.namespaced_base_name == 'std::optional': result = StdOptionalStructure(arg=type.template_args[0])
  elif type.namespaced_base_name == 'std::array': result = StdArrayStructure(arg=type.template_args[0], size=int(type.template_args[1].base_name))
  elif type.namespaced_base_name == 'std::pair': result = StdPairStructure(arg1=type.template_args[0], arg2=type.template_args[1])

  if result is not None: _used_builtins[result.base_name] = result
  return result
...
class BuiltinStructure(c.Structure):
  ...
  @property
  def template_file_stem(self) -> str: return self.base_name
