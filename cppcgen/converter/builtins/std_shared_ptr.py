...
@dataclass
class StdSharedPtrStructure(BuiltinStructure):
  arg: cpp.Type

  @property
  def cpp_structure_impl(self):
    return cpp.Structure(
      name=f'shared_ptr<{self.arg.full_name}>',
      namespace='std',
      c_structure_=self,
    )
  ...
  @property
  def base_name_impl(self) -> str: return f'std_shared_ptr_{self.arg.as_c_ptr.base_name.lower()}'
